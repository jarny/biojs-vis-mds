...
var selectRows = function(allOrNone) {
...
function drawDistLinePlot(d,i) {
  ...
  for (var j=0; j<distancesSubset[i].length; j++) {
    data.push({'name':samplesSubset[j][$scope.selectedSampleGroup],
               'distance':distancesSubset[i][j],
               'colour':colourFromSampleGroup[$scope.selectedSampleGroup][samplesSubset[j][$scope.selectedSampleGroup]]});
  }
...
var rowMouseover = function(rowIndex,evt) {
...
var rowMouseout = function(rowIndex) {
  var sampleGroupItem = $scope.rows[rowIndex].sampleGroupItem;
  CommonService.hideTooltip();
...
var saveFigure = function(figureIndex) {
...
var closeSaveFigureDialog = function() { $scope.showSaveFigureDialog = false; };
